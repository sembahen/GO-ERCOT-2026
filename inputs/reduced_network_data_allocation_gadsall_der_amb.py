...
#lostcaptype
lostcaptype="gads_all"
#ambient derates
lostcaptype2="gads_all_DerAmb"
...
for NN in NODE_NUMBER:
    ...
    for UC in UC_TREATMENTS:
        ...
        for T_p in trans_p:
            ...
            names1 = list(df_gens['BusName'])
            fts = list(df_gens['FuelType'])
            names_hr1 = list(df_gens_heat_rate['BusName'])
            ...
            for n in names_hr1:
                ...
                corrected = corrected + '_' + f
                names_hr1[i] = corrected
          
            df_gens['PlantNames'] = names1
            df_gens_heat_rate['PlantNames'] = names_hr1            
            ...
            for n in NB:
                #subset the df generators by NEW BUS NUMBER
                sample = df_gens.loc[df_gens['NewBusNum'] == n]
                sample_hr = df_gens_heat_rate.loc[df_gens_heat_rate['NewBusNum'] == n]
                #for every NEW BUS NUMBER, after pulling the dataframe by each new bus num, find the UNIQUE PLANT NAMES
                sublist = sample['PlantNames'].unique()
                for s in sublist:
                    #for each unique plant name,find the fuel type
            ...
            #datagen params excludes nuclear and includes solar, wind and hydro
            ...
            #copy other files
            w = 'wrapper' + UC + '_{}'.format(lostcaptype2) +'.py'
            milp = 'ERCOT_MILP' + UC + '_{}'.format(lostcaptype) +'.py'
            lp = 'ERCOT_LP' + UC + '_{}'.format(lostcaptype) +'.py'
            
            copy(w,path)
            copy('ERCOTDataSetup' + '_{}'.format(lostcaptype) +'.py',path)
            
            if UC == '_simple':
                copy('ERCOT' + UC + '_{}'.format(lostcaptype) +'.py',path)
            else:          
                copy(milp,path)
                copy(lp,path)
            ...
            #copy lost capacity dataframe
            copy("ercotGensGadsallLost_DerAmbV4_1_MW.csv", path)
            
            
            
            
            #................................................................................................................................................................................................................
            #BELOW IS CODE TO MAP THE REDUCED THERMAL GENS TO THE PLANTS THEY BELONG TO IN THE REDUCED NETWORK
            '''DF GENS THERMAL''' #HENRYSSEMBATYA
            #subset the df_gens to only retain the thermal gens
            df_gens_full_thermal=df_gens[df_gens['FuelType'].isin(thermal)].reset_index(drop=True)
            
            #class category of every plany
            def gencat(row):  
                if row['MWMax'] <= 50 :
                    return "below_50"
                elif row['MWMax'] <= 100:
                    ...
                    return "ovr_1000"
            #adding a column for the class category
            df_gens_full_thermal['gen_categ'] = df_gens_full_thermal.apply(lambda row: gencat(row), axis=1)
            #defining the fuel type
            df_gens_full_thermal.loc[:,"GenFuelType"]= df_gens_full_thermal["FuelType"].replace({"NG (Natural Gas)": "Gas" ,"NUC (Nuclear)": "Nuclear" , "BIT (Bituminous Coal)" : "Coal"})
            #defining the class category
            df_gens_full_thermal.loc[:,"ClassCategory"]=df_gens_full_thermal.loc[:,"GenFuelType"]  + "_" + df_gens_full_thermal.loc[:,'gen_categ']
            
            
            
            #function to truncate a string until the last character is neither a number nor an underscore
            def truncate_trailing_numbers_and_underscores(s):
                # Keep truncating while the last character is a digit or an underscore
                while s and (s[-1].isdigit() or s[-1] == '_'):
                    s = s[:-1]  # Remove the last character
                return s
            
            
            
            #FROM DF_C (create a copy of the ERCOT generators dataframe)
            df_reduced_thermalgens=df_C.copy()
            #create a list to save the plant name before the change
            plant_names_be4=[]
            
            #create a list of the new plant names in the reduced thermalgens dataframe
            reduced_plantname = df_reduced_thermalgens['Name'].to_list()

            #for every plant name in the reduced thermalgens dataframe                
            for i in reduced_plantname:
                #remove trailing numbers and underscores (to make the names similar to the thermal gens full dataframe)
                plantnames_change=truncate_trailing_numbers_and_underscores(i)
                #add to new list without trailing numbers and underscores
                plant_names_be4.append(plantnames_change)
            #in the new reduced thermal gens dataframe, add the "original plant names"
            df_reduced_thermalgens["PlantName_Before"]=plant_names_be4
            
            
            #new plant name in the reduced network, mapped to the original plant name in all thermal gens
            plant_name_reduced=[]
            for i in range(len(df_gens_full_thermal)):
                #obtaining the newbus num and the plant name in the full thermal gens dataframe
                newbusnum_full=df_gens_full_thermal.loc[i, 'NewBusNum']
                plantname_full=df_gens_full_thermal.loc[i, 'PlantNames']
                
                #going into the reduced thermal gens and finding that newbusnum
                plantdata_sample = df_reduced_thermalgens.loc[df_reduced_thermalgens['Bus'] == str(newbusnum_full)]
                #getting the orifinal plant name 
                plantname_sample = plantdata_sample.loc[plantdata_sample['PlantName_Before'] == plantname_full]
                
                #adding that plant name to a list
                plant_name_reduced.append(plantname_sample['Name'].values[0])
            #saving that list as a column to the full thermal gens dataframe
            df_gens_full_thermal["PlantName_Reduced"]=plant_name_reduced
            
            #creating a smaller dataset with only the features we need
            df_thermalgens_full_subset=df_gens_full_thermal[["BusName", "NewBusNum", "ClassCategory", "PlantName_Reduced", "GenFuelType", "MWMax"]]
            df_thermalgens_full_subset.loc[:,'BusName'] = df_thermalgens_full_subset.loc[:,'BusName'].str.replace(' ', '_', regex=False)
            df_thermalgens_full_subset.to_csv("thermalgens_full.csv", index=None)
            copy("thermalgens_full.csv",path)
